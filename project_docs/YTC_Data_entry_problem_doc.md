...
| **FR-2.6** | Filter dashboard views by model. When multiple models run simultaneously, the Daily Summary and Pipeline views aggregate all models per station, making attainment figures misleading. A model selector lets supervisors isolate one model's station-by-station performance. The Model Progress view already operates at the model level and does not require this filter. | P0 |
